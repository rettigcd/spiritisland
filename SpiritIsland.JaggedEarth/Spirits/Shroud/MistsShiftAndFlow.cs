...
class MistsShiftAndFlow {
	...
	readonly TargetSourceCriteria _sourceCriteria;
	...
	public MistsShiftAndFlow(SelfCtx ctx, string prompt, TargetSourceCriteria sourceCriteria, TargetCriteria[] targetCriteria, TargetingPowerType targettingFrom) {
	...
	async Task FlowPresence( SpaceState target ) {
		...
		var gatherDst = await _spirit.Action.Decision( new Select.Space(
		...
		var gatherSource = await _spirit.Action.Decision( Select.DeployedPresence.Gather( $"Flow (gather) presence (to {gatherDst.Label}) from:", gatherDst, souceOptions ) );
	...
	bool PresenceMeetsTargettingRequirements( IKnowSpiritLocations presence, SpaceState target ) {
		var targetSource = _spirit.SourceCalc.FindSources( presence, _sourceCriteria, _gameState );
	...
	async Task<SpaceState> FindTarget() {
		...
		var target = await _spirit.Action.Decision( new Select.Space( _prompt, nonFlowTargets.Union( flowOnlyTargets ), Present.Always ) );
