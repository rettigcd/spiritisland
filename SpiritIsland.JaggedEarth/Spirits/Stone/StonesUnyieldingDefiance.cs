...
public StonesUnyieldingDefiance() : base(
		...
		this.GrowthTrack = new GrowthTrack(
			new GrowthOption(new ReclaimAll(),new PlacePresenceOnSelf( 3,Target.Mountain, Target.Presence ),new GainElements(Element.Earth,Element.Earth)),
	...
	void BestowTheEnduranceOfBedrock( AddBlightEffect effect ) {
		...
		if( effect.AddedTo.Blight.Count <= Presence.CountOn( effect.AddedTo ) ){
