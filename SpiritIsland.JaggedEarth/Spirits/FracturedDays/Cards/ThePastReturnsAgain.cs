...
class ThePastReturnsAgain {
	...
	static public async Task ActAsync( Spirit self ) {
		...
		if(! await GetEnergyFromEachSpirit( self, gs.Spirits.Length ) ) return;
		...
		var oldCard = await self.SelectAsync( new A.TypedDecision<InvaderCard>(
	...
	static async Task<bool> GetEnergyFromEachSpirit( Spirit self, int total ) {
		int remaining = total;
		...
		} while(0 < remaining && await self.UserSelectsFirstText( $"{remaining} of {total} remaining.", "Pass the hat again.", "No, this isn't going to work." ));
