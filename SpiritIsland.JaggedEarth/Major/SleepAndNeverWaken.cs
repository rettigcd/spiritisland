...
public class SleepAndNeverWaken {
	...
	public static async Task ActAsync(TargetSpaceCtx ctx ) {
		...
		if( await ctx.YouHave( "3 moon,2 air,2 animal") )
			await RemoveExploreres( ctx, 6, ctx.Self.Presence.ActiveSpaceStates.Downgrade().ToArray() );
	...
	static async Task RemoveExploreres( TargetSpaceCtx ctx, int count, params Space[] fromSpaces ) {
		...
		while( count-- > 0 
			...
			var spaceToken = await ctx.Decision( new Select.TokenFromManySpaces($"Select Explorer to remove. ({count+1} remaining)", options, Present.Done));
