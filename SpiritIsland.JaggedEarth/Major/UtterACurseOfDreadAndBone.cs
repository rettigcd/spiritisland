...
public class UtterACurseOfDreadAndBone {
	...
	public static async Task ActAsync(TargetSpaceCtx ctx ) {
		...
		for(int i = 0; i < blightCount; i++)
			...
			await ctx.SelectActionOption(
				new SpaceCmd("Add Badland", async ctx=>{ await ctx.Badlands.AddAsync(1); ++badland; } ).OnlyExecuteIf(badland<3),
				new SpaceCmd("Add Disease", async ctx=>{ await ctx.Disease.AddAsync(1); ++disease; } ).OnlyExecuteIf(disease<3),
	...
	static async Task AddTokenToLandWithinRange( TargetSpaceCtx ctx, IEntityClass tokenClass, int range ) {
		var space = await ctx.Decision( new Select.ASpace( $"Add {tokenClass.Label}", ctx.Range( range ), Present.Always ) );
	...
	static async Task AddStrifeToLandWithinRange( TargetSpaceCtx ctx, int range ) {
		var space = await ctx.Decision(new Select.ASpace("Add Strife",ctx.Range(range), Present.Always));
	...
	static async Task DamageAdjacentLand( TargetSpaceCtx ctx ) {
		...
		var space = await ctx.Decision(new Select.ASpace("Select land for 1 Damage", ctx.Adjacent, Present.Always));
