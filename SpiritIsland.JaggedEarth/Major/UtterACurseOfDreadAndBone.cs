		...
		public static async Task ActAsync(TargetSpaceCtx ctx ) {
			...
			for(int i = 0; i < blightCount; i++)
				...
				await ctx.SelectActionOption(
					new SpaceAction("Add Badland", async ctx=>{ await ctx.Badlands.Add(1); ++badland; } ).Cond(badland<3),
					new SpaceAction("Add Disease", async ctx=>{ await ctx.Disease.Add(1); ++disease; } ).Cond(disease<3),
					new SpaceAction("Add Strife", async ctx=>{ await ctx.AddStrife(); ++strife; } ).Cond(strife<3)
				);
