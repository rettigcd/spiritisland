...
public class FireStorm {
	...
	private static async Task DoFireDamageToMultipleTargets( TargetSpaceCtx ctx, int fireDamage ) {
		...
		while(0 < fireDamage && invaderTokens.Any()) {
			SpaceToken token = await ctx.Self.SelectAlways( $"Apply fire damage. ({fireDamage} remaining)",invaderTokens);
	...
	static public async Task Option4( TargetSpaceCtx ctx ) {
		...
		var space = await ctx.Self.Select( new A.SpaceDecision($"Push all dahan, destroy invaders and beast, 1 blight",spacesWithPresenceAndBlight,Present.Always));
