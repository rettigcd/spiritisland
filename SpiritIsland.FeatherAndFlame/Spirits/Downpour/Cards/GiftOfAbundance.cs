...
internal class GiftOfAbundance {
	...
	static public async Task ActAsync( TargetSpiritCtx ctx ) {
		...
		var spiritsWithPresenceInWetland = new[] { ctx, ctx.OtherCtx }
			.Distinct() // if solo
			.Where( ctx => 0<ctx.Self.Presence.Destroyed && ctx.Presence.ActiveSpaceStates.Any(isWetland));
		...
		var spaceOptions = restoringSpiritCtx.Presence.ActiveSpaceStates.Where(isWetland);
