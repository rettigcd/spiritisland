...
public class SpreadingTimidity : FearCardBase, IFearCard {
	...
	public Task Level2( GameCtx ctx ) => new SpaceAction( $"Isolate target land and Defend 2.", ctx => { ctx.Isolate(); ctx.Defend( 2 ); } )
		.In().SpiritPickedLand().AllDifferent()
		.ForEachSpirit()
		.ActAsync( ctx );

	[FearLevel( 3, "Each player chooses a different land to Isolate. Also, Defend 4 in those lands." )]
	public Task Level3( GameCtx ctx ) => new SpaceAction( $"Isolate target land and Defend 4.", ctx => { ctx.Isolate(); ctx.Defend( 4 ); } )
		.In().SpiritPickedLand().AllDifferent()
		.ForEachSpirit()
		.ActAsync( ctx );
}
