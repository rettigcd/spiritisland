...
public class MimicTheDahan : FearCardBase, IFearCard {
	...
	public override Task Level1( GameState ctx ) => Cmd.RemoveExplorersOrTowns(1)
...
//		.ByPickingToken(Human.Explorer_Town)// Generating an Exceptoin
	...
	public override Task Level2( GameState ctx ) => ReplaceExplorerOrTownWith1Dahan
		.In().SpiritPickedLand().Which( Has.TwoOrMoreDahan )
//		.ByPickingToken( Human.Explorer_Town ) // Generating an Exceptoin
		.ForEachSpirit()
		.ActAsync( ctx );
	...
	public override Task Level3( GameState ctx ) => ReplaceExplorerOrTownWith1Dahan
