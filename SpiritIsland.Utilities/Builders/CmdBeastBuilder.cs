...
namespace SpiritIsland.Utilities.Builders; 
...
internal class CmdBeastBuilder {
	...
	static public Bitmap ForStage( int stage ) {
		...
		new PoolRect()
			// rect: border
			.Float( new RectRect{ Fill = "bab4a6", Stroke = "363636", }.RoundCorners(.1f), 0f,0f,1f,1f )
			// rect: pink
			...
			.Float( new TextRect( stage==1?"STAGE I": "STAGE II" ), 0f, .2f, 1f, .12f )
