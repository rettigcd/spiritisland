...
public static class GrowthActionBuilder {
	...
	public static IPaintableRect GetGrowthPaintable( IActOn<Spirit> action ) {
		...
		return action switch {
			...
			"Place Presence and Beast" => PlacePresenceRect( action ),
			// Wounded Waters Bleeding
			"Add Destroyed Presence-Range 1" => PlacePresenceRect( action ),
			// Ocean
			"Place in Ocean" => PlacePresenceRect( action ),
			...
			"Energy for Fire" => new ImgRect( Img.Oneenergyfire ),
			// Fractured Dates
			"Gain Time(2)" => new ImgRect( Img.FracturedDays_Gain2Time ),
			"Gain Time(1)x2" => new ImgRect( Img.FracturedDays_Gain1Timex2 ),
			"Gain Time(1)x3" => new ImgRect( Img.FracturedDays_Gain1Timex3 ),
			"Draw Power Card from Days That Never Were" => new ImgRect( Img.FracturedDays_DrawDtnw ),
			// Starlight Seeks Its Form
			"Make Power Fast" => new ImgRect( Img.Icon_Fast ),
			// Grinning Trickster
			"Gain Energy Equal to Card Plays" => new ImgRect( Img.GainEnergyEqualToCardPlays ),
			// Many Minds
			"Gather 1 Token" => new ImgRect( Img.Land_Gather_Beasts ),
			"Apply Damage" => new ImgRect( Img.Damage_2 ),
			"Discard 2 Power Cards" => new ImgRect( Img.Discard2 ),
			// Towering Roots
			"Add Vitality to Incarna" => AddVitalityToIncarna(),
			"Replace Presence with Incarna" => ReplacePresenceWithIncarna(),
			// Finder
			"Ignore Range" => Draw_IgnoreRange(),
			// Relentless Gaze of the Sun
			"Gain Energy an additional time" => GainEnergyAgain(),
			"Move up to 3 Presence together" => MoveUpTo3PresenceTogether(),
			// Dances up Earthquakes
			"Add Presence or Gain Major" => AddPresenceOrGainMajor(),
			"Accelerate or Delay" => AccelerateOrDelay(),
			// Breath of Darkness
			"1 piece Escapes" => PiecesEscape( 1 ),
	...
	static IPaintableRect GetTargetFilterIcon( string filterEnum ) {
		...
		return orParts.Length == 2
			...
			: new ImgRect(GetImgEnum(filterEnum));
	}
