...
public class UnbearableGaze {
	...
	static async Task PushFromTargetOrOrigin( TargetSpaceCtx ctx, int max, params HumanTokenClass[] tokenClasses ) {
		...
		for(int i = 0; i < max; ++i) {
			...
			// Select 1 to push
			var toPush = await ctx.Self.Gateway.Decision( new Select.ASpaceToken( "Select Explorer/Town to push", tokenOptions, Present.Always ) );
	...
	static async Task PushSpecificToken( Spirit spirit, SpaceToken toPush ) {
		// Push it
		Space destination = await spirit.Gateway.Decision( Select.ASpace.PushToken(
