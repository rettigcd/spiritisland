...
public class ReachFromTheInfiniteDarkness {
	...
	static async Task<(SpaceToken?,Spirit?)> SelectPresenceToAbduct( SelfCtx ctx, int remaining ) {
		...
		var presenceToAbduct = await ctx.Decision( new A.SpaceToken( $"Abduct Presence for +1 Range for all powers ({remaining} remaining)", spiritLookup.Keys, Present.Done ) );
