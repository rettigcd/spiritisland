...
namespace SpiritIsland.WinForms {
	...
	sealed public class FearCardImageManager {
		...
		public static Image GetImage( IFearCard card ) {
			...
			// Text 1..3
			// !!! Align Center Horizontally and align vertically
			// !!! 1st try narrower width, then if that doesn't work, use full width.
			...
			for(int i = 0; i < 3; ++i) {
				...
				var rowSize = new Size( textBounds.Width - widthReduction, rowHeight );
				var layout = new WrappingLayout( emSize,rowSize, textBounds.Location, graphics ) {
				...
				while( 0 < widthReduction && 2 < layout.RowCount) {
					widthReduction -= 20;
					// generate with stepped-in width
				...
				int remainingHeight = outterArea.Height - layout.Bounds.Height;
				int adjustX = (textBounds.Width - rowSize.Width) / 2;
				int adjustY = remainingHeight / 2;
				layout.PrivateAdjust( adjustX, adjustY );

				layout.Paint( graphics );
			}


			var debug = new System.Collections.Generic.Dictionary<string, Rectangle> {
				//["inner"] = innerBounds,
				//["title"] = titleArea,
				//["canvas"] = canvasArea,
				//["sec-1"] = secRow1,
			...
			foreach(var pair in debug) {
				graphics.DrawRectangle(Pens.Blue,pair.Value);
				graphics.DrawString(pair.Key, SystemFonts.MenuFont, Brushes.Blue, (float)pair.Value.X, (float)pair.Value.Y );
			}
