...
public class France : IAdversary {
	...
	static BaseCmd<BoardCtx> AddStrifeToTown => new BaseCmd<BoardCtx>(
		...
		, async boardCtx => {
			...
			var st = await boardCtx.Decision( new A.SpaceToken( "Add strife to town", options, Present.Always ) );
			if(st != null)
	...
	static BaseCmd<BoardCtx> Add2StrifeToCityOrTown => new BaseCmd<BoardCtx>(
		...
		, async boardCtx => {
			...
			for(int i = 0; i < 2; ++i) {
				var st = await boardCtx.Decision( new A.SpaceToken( $"Add strife ({i+1} of 2)", options, Present.Always ) );
				if(st != null)
					await st.Space.Tokens.Add1StrifeTo( st.Token.AsHuman() );
			}
		} );
	...
	static BaseCmd<BoardCtx> DestroyTown => new BaseCmd<BoardCtx>(
		...
		, async boardCtx => {
			SpaceToken[] options = boardCtx.Board.FindTokens( Human.Town );
			var st = await boardCtx.Decision( new A.SpaceToken( "Destory a town", options, Present.Always ) );
			if(st != null)
