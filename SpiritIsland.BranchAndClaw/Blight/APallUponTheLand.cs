	...
	static BaseCmd<BoardCtx> Destroy1PresenceOnBoard 
		=> new BaseCmd<BoardCtx>("Destroy 1 presence.", async boardCtx => {
			...
			var spirit = spiritOptions.Length == 1 ? spiritOptions[0]
				: await boardCtx.Self.SelectAlways( "Select Spirit to Destroy 1 presence", spiritOptions );

			var spaceToken = await spirit.SelectAlways( "Select Presence to Destory", spirit.Presence.Deployed.WhereIsOn(boardCtx.Board.Spaces.ScopeTokens()) );
