	...
	public class BloodwrackPlague {
		...
		static public async Task ActAsync(TargetSpaceCtx ctx ) {
			...
			if(ctx.YouHave("2 earth,4 animal")) { 
				...
				var space = await ctx.Self.Action.Decide(new TypedDecision<Space>($"Select space to apply {damage} damage", ctx.Space.Range(1), Present.Always )); // can we wrap this and make it easier to call?
