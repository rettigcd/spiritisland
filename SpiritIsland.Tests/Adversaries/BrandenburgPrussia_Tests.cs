...
public class BrandenburgPrussia_Tests {
	...
	void SetAdversary(int level) {
		var gameConfig = new GameConfiguration().ConfigSpirits(RiverSurges.Name).ConfigBoards("A");
		gameConfig.Adversary = new AdversaryConfig(BrandenburgPrussia.Name, level);
		TestGames.GameBuilder.BuildGame(gameConfig);
	}

	void Assert_Level1TownAdded(bool added ) 
		=> GameState.Current.Island.Boards[0][3].ScopeSpace.Summary.ShouldBe(added ? "2D@2,1T@2" : "[none]");

	void Assert_FearLevels(int v1, int v2, int v3 ) {
		var fear = GameState.Current.Fear;
		// level 1
		for(int i = 0; i < v1; ++i) {
			fear.TerrorLevel.ShouldBe(1);
			fear.Deck.Pop();
		}
		// level 2
		for(int i = 0; i < v2; ++i) {
			fear.TerrorLevel.ShouldBe( 2 );
			fear.Deck.Pop();
		}
		// level 3
		for(int i = 0; i < v3; ++i) {
		...
		// last card
		fear.Deck.Count.ShouldBe( 0 );
	}

	const string defaultInvaderDeck = "111-2222-33333";
	void Assert_InvaderDeck(string expectedLevelsString ) {
		string actualInvaderLevels = GetActualInvaderLevels( GameState.Current.InvaderDeck );
