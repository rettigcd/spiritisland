...
public class CallToTrade_Tests {
	...
	public void NoRavage_NoBuild() {
		...
		var (user, self, _) = TestSpirit.StartGame( PowerCard.For(typeof(CallToTrade)) );
	...
	public void OneRavage_ReplacedWithBuild() {
		var (user, ctx, _) = TestSpirit.StartGame( PowerCard.For(typeof(CallToTrade)) );
	...
	public void BuildAndRavage_BecomesTwoBuilds() {
		...
		var (user, ctx, _) = TestSpirit.StartGame( PowerCard.For(typeof(CallToTrade)), (Action<GameState>)((gs)=>{ 
