...
public class OceanTerrain_Tests {
	...
	public async Task PushDahanIntoOcean(bool withOcean) {
		...
		await using UnitOfWork action = gameState.StartAction( ActionCategory.Spirit_Power ); // !!! needed?
		SelfCtx ctx = primarySpirit.BindMyPowers( gameState );
	...
	void Given_PrimaryPresenceOnlyOn( Space space ) {
		foreach(SpaceState ss in primarySpirit.Presence.ActiveSpaceStates.ToArray())
		...
		primarySpirit.Presence.ActiveSpaceStates.SelectLabels().Join( "," ).ShouldBe( space.Text );
	}
