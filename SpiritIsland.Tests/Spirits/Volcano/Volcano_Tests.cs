...
public class Volcano_Tests {
	...
	public async Task Range(int presenceCount, string expectedOptions ) {
		...
		_ = PowerCard.For<MesmerizedTranquility>().ActivateAsync( spirit.BindMyPower(gameState, uow) );
	...
	public void PowerDestroyingPresence_CausesDamage() {
		...
		using UnitOfWork uow = gameState.StartAction( ActionCategory.Spirit_Power );
		Task task = PowerCard.For<GrowthThroughSacrifice>().ActivateAsync( spirit.BindMyPower( gameState, uow ) );
	...
	public void BargainsOfPowerAndProtection_UsesExtendedRangeOnDahan() {
		...
		spirit.Presence.Adjust( targetSpace, 5 );
		...
		//  When: they destroying presence via Powercard
		using UnitOfWork uow = gameState.StartAction( ActionCategory.Spirit_Power );
		Task task = PowerCard.For<BargainsOfPowerAndProtection>().ActivateAsync( spirit.BindMyPower( gameState, uow ) );
	...
	public void UtterACurseOfDreadAndBone_ExtendedRange(int towerSpaceNum, string expectedRangeOptions) {
		...
		// Given: Enough Presence to trigger Tower rule
		spirit.Presence.Adjust( towerSpace, 5 );
		...
		//  When: Utter a curse
		using UnitOfWork uow = gameState.StartAction( ActionCategory.Spirit_Power );
		Task task = PowerCard.For<UtterACurseOfDreadAndBone>().ActivateAsync( spirit.BindMyPower( gameState, uow ) );
	...
	public void UnleashATorrent_ExtendedRange( int presenceInTower, string expectedRangeOptions ) {
		...
		spirit.Presence.Adjust( space, presenceInTower );
		//   And: 1 energy
		spirit.Energy=1;

		//  When: Unleash a Torrent
		using UnitOfWork uow = gameState.StartAction( ActionCategory.Spirit_Power );
		Task task = PowerCard.For<UnleashATorrentOfTheSelfsOwnEssence>().ActivateAsync( spirit.BindMyPower( gameState, uow ) );
	...
	[InlineData( 6 )]
	[InlineData( 10 )]
	public void ExplosiveEruption( int presenceDestroyed ) {
		var (spirit, gameState, board) = Init();
		SpaceState targetSpace = gameState.Tokens[board[8]];
