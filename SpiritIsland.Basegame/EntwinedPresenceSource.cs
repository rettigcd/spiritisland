...
namespace SpiritIsland.Basegame {
	...
	class EntwinedPresenceSource : ICalcSource {
		...
		public IEnumerable<Space> FindSources( Spirit self, From sourceEnum, Terrain? sourceTerrain ) {
			List<Space> sources = new ();
			// Find source of original
			for(int i = 0; i < spirits.Length; ++i)
				sources.AddRange( origApis[i].FindSources( spirits[i], sourceEnum, sourceTerrain ) );
