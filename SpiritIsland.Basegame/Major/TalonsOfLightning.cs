...
public class TalonsOfLightning {
	...
	[MajorCard( "Talons of Lightning", 6, Element.Fire, Element.Air ),Fast,ExtendableRange( TargetFrom.Presence, 1, "3 fire,3 air", 2, Target.Mountain, Target.Wetland )]
	...
	static public async Task ActionAsync( TargetSpaceCtx ctx ) {
		...
		if( await ctx.YouHave("3 fire,3 air" )){
			...
			foreach(var neighbor in ctx.Tokens.Adjacent)
				await ctx.Target(neighbor.Space).Invaders.DestroyNOfClass( 1,Human.Town);
