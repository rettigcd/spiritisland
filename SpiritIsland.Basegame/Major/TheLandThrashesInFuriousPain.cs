...
public class TheLandThrashesInFuriousPain {
	...
	static public async Task ActAsync(TargetSpaceCtx ctx) {
		...
		if(await ctx.YouHave("3 moon,3 earth")) {
			...
			var alsoTarget = await ctx.Decision( new Select.Space( "Select additional land to receive blight damage", ctx.Tokens.Adjacent.Select(s=>s.Space), Present.Always));
