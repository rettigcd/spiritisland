...
namespace SpiritIsland.Basegame {
	...
	public class InfiniteVitality {
		...
		static async Task RemoveBlightFromLandOrAdjacent( TargetSpaceCtx ctx ) {
			...
			var blightedLands = ctx.Space.Range( 1 ).Where( ctx.GameState.HasBlight ).ToArray();
			var unblightLand = await ctx.Self.Action.Decide( new TargetSpaceDecision( "Remove 1 blight from", blightedLands ));
