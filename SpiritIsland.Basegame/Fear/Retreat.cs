...
public class Retreat : FearCardBase, IFearCard {
	...
	public Task Level2( GameCtx ctx ) {
		return Cmd.EachSpirit( Cmd.PushUpToNInvaders( 3, Invader.Explorer_Town ).From( x => !x.IsCoastal && x.IsInPlay, "Inland" ) ) // !! assuming Explorers and Town are the same.
			.Execute( ctx );
	}

	[FearLevel( 3, "Each player may Push any number of Explorer / Town from one land." )]
	public Task Level3( GameCtx ctx ) {
		return Cmd.EachSpirit( Cmd.PushUpToNInvaders( int.MaxValue, Invader.Explorer_Town ).From( _ => true, "" ))
				.Execute( ctx );
	}
