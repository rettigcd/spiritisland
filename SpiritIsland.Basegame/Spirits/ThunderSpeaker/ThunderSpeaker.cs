	...
	*/
	...
	public class Thunderspeaker : Spirit {
		...
		protected override void InitializeInternal( Board board, GameState gs ) {
			...
			// Special Rules -Ally of the Dahan - Your presense may move with dahan
			gs.Tokens.TokenMoved.ForEntireGame( new MovePresenceWithTokens( this, "Move presence with dahan?", TokenType.Dahan ).CheckForMove );
		...
		async Task DestroyNearbyPresence( GameState _, TokenDestroyedArgs args ) {
			...
			while(numToDestroy-->0 && (options=Calc()).Length > 0)
				Presence.Destroy( await this.Action.Decision( new Decision.Presence.Deployed( prompt, options, Present.Always ) ));
