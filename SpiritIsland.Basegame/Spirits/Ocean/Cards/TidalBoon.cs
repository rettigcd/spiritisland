...
namespace SpiritIsland.Basegame {
	...
	public class TidalBoon {
		...
		static public async Task Act(TargetSpiritCtx ctx ) {
			...
			var pushLand = await ctx.Other.Action.Decide( new TargetSpaceDecision( "Select land to push town and 2 dahan", ctx.Other.Presence.Spaces, Present.Done ));
			if(pushLand==null) return;

			var spaceCtx = new TargetSpaceCtx(ctx.Other,ctx.GameState,pushLand);
			await spaceCtx.PushUpTo(1,Invader.Town);
			await spaceCtx.PushUpTo(2,TokenType.Dahan);
			...
			async Task PushDahanOutOfOcean( GameState gs, TokenMovedArgs args ) {
				...
				await ctx.Self.MakeDecisionsFor(gs).PushUpToN(args.to,args.count,TokenType.Dahan);
			}
