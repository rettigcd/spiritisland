...
public class ChokeTheLandWithGreen : SpiritPresenceToken , ISkipBuilds, ISkipRavages  {
	...
	async Task<bool> SkipInvaderAction( GameState gs, SpaceState space, string actionDescription ) {
		...
		var unitOfWork = gs.StartAction( ActionCategory.Spirit_SpecialRule ); // Special Rules!
		await _self.Presence.Destroy( stop, gs, 1, DestoryPresenceCause.SkipInvaderAction, unitOfWork ); // it is the invader actions we are stopping
