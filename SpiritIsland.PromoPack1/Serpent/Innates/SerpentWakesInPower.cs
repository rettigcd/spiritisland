...
public class SerpentWakesInPower {
	...
	static public async Task Option2Async( SelfCtx ctx ) {
		...
		await ctx.Presence.PlaceWithin(ctx.TerrainMapper.Specify(1),TargetingPowerType.Innate);
		...
		foreach(var spirit in presence.AbsorbedPresences.Distinct())
			await ctx.NewSelf(spirit).Presence.PlaceWithin(ctx.TerrainMapper.Specify(1),TargetingPowerType.Innate);
