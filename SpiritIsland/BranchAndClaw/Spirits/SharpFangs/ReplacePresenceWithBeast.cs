...
namespace SpiritIsland.BranchAndClaw {
	class ReplacePresenceWithBeast : GrowthActionFactory {
		...
		public override async Task ActivateAsync( Spirit spirit, GameState gameState ) {
			...
			var space = await spirit.Action.Choose(new TargetSpaceDecision("Select presence to replace with beast",options,Present.Done)); // let them change their minds
			if(space == null) return;

			spirit.Presence.RemoveFrom(space);
			(gameState as GameState_BranchAndClaw).Beasts.AddOneTo(space);
