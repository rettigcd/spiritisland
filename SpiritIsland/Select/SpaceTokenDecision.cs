...
public class MyTokenOn( string prompt, IEnumerable<ITokenLocation> options, Present presenct ) 
	: TypedDecision<ITokenLocation>( prompt, options, presenct )
{
	public ITokenLocation[] TokensOn { get; } = options.ToArray();
}