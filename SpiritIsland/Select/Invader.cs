...
public static class Invader {
	...
	public static TokenFromManySpaces ToReplace( string actionVerb, SpiritIsland.Space space, IEnumerable<ISpaceEntity> options, Present present = Present.Always ) 
		=> new TokenFromManySpaces( $"Select invader to {actionVerb}", space, options.Cast<HumanToken>(), present );

	public static TokenFromManySpaces ToRemove( SpiritIsland.Space space, IEnumerable<IToken> options ) 
	...
	public static TokenFromManySpaces ForIndividualDamage(int damagePerInvader, SpiritIsland.Space space, IEnumerable<IToken> invaders)
		=> new TokenFromManySpaces( $"Select invader to apply {damagePerInvader} damage", space, invaders.Distinct(), Present.Done );

	public static TokenFromManySpaces ForAggregateDamage( SpiritIsland.Space space, IToken[] invaderTokens, int aggregateDamage, Present present) 
		=> new TokenFromManySpaces( $"Damage ({aggregateDamage} remaining)",space, invaderTokens, present );

	public static TokenFromManySpaces ForAggregateDamageFromSource( SpiritIsland.Space space, HumanToken source, IToken[] invaderTokens, int aggregateDamage, Present present )
		=> new TokenFromManySpaces( $"Damage from {source} ({aggregateDamage} remaining)", space, invaderTokens, present );

	public static TokenFromManySpaces ForBadlandDamage(int remainingDamage, SpiritIsland.Space space, IEnumerable<IToken> invaders)
