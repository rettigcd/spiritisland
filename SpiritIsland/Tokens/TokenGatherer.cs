	...
	public class TokenGatherer {
		...
		async Task Gather_Inner( Present present ) {
			...
			while(0 < (options = GetOptions( RemainingTypes )).Length) {
				...
				--sharedGroupCounts[indexLookupByGroup[source.Token.Generic]];
			}
		}

		TokenGroup[] RemainingTypes => indexLookupByGroup
			.Where( pair => sharedGroupCounts[pair.Value] > 0)
			.Select( pair => pair.Key )
			.ToArray();

		public TokenGatherer AddGroup( int countToGather, params TokenGroup[] groups ) {
