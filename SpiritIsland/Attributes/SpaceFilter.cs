...
namespace SpiritIsland {
	...
	public class SpaceFilter {
		...
		public static readonly Dictionary<string,Func<SpaceFilterCtx, Space,bool>> lookup = new Dictionary<string, Func<SpaceFilterCtx, Space, bool>> {
			[Target.Any               ] = ( _, s ) => true,
			[Target.SandOrWetland     ] = ( ctx, s ) => ctx.SelectTerrain( s ).IsOneOf( Terrain.Sand, Terrain.Wetland ),
			...
			[Target.JungleOrMountain  ] = ( ctx, s ) => ctx.SelectTerrain( s ).IsOneOf( Terrain.Jungle, Terrain.Mountain ),
			[Target.JungleOrSand      ] = ( ctx, s ) => ctx.SelectTerrain( s ).IsOneOf( Terrain.Jungle, Terrain.Sand ),
			[Target.JungleOrWetland   ] = ( ctx, s ) => ctx.SelectTerrain( s ).IsOneOf( Terrain.Jungle, Terrain.Wetland ),
			[Target.MountainOrWetland ] = ( ctx, s ) => ctx.SelectTerrain( s ).IsOneOf( Terrain.Mountain, Terrain.Wetland ),
