...
public static class Has {
	...
	static public TargetSpaceCtxFilter Only1ExplorerTown => new TargetSpaceCtxFilter( "has only 1 explorer or town", x => x.Tokens.SumAny( Human.Explorer_Town ) == 1 && !x.Tokens.Has( Human.City ) );
	static public TargetSpaceCtxFilter TwoOrFewerInvaders => new TargetSpaceCtxFilter( "has 2 or fewer invaders", x => x.Tokens.SumAny( Human.Invader ) <= 2 );
	static public TargetSpaceCtxFilter TwoOrMoreInvaders => new TargetSpaceCtxFilter( "has at least 2 invaders", x => 2 <= x.Tokens.SumAny( Human.Invader ) );
	static public TargetSpaceCtxFilter TownOrCity => new TargetSpaceCtxFilter( "has town/city", ctx => ctx.Tokens.HasAny( Human.Town_City ) );
	static public TargetSpaceCtxFilter NoCity => new TargetSpaceCtxFilter( "has no City", x => !x.Tokens.Has( Human.City ) );
	static public TargetSpaceCtxFilter City => new TargetSpaceCtxFilter( "has city", x => x.Tokens.Has( Human.City ) );
	...
	static public TargetSpaceCtxFilter Beast => Token( SpiritIsland.Token.Beast);
	...
	// Dahan / Invader Mix
	static public TargetSpaceCtxFilter DahanAndExplorerOrTown => new TargetSpaceCtxFilter( "has dahan", ctx => ctx.Tokens.Dahan.Any && ctx.Tokens.HasAny( Human.Explorer_Town ) );
	static public TargetSpaceCtxFilter Two2DahanAndCity => new TargetSpaceCtxFilter( "has 2 dahan", ctx => 2 <= ctx.Tokens.Dahan.CountAll && ctx.Tokens.Has( Human.City ) );
	static public TargetSpaceCtxFilter DahanAndExplorers => new TargetSpaceCtxFilter( "land with Dahan", x => x.Dahan.Any && x.Tokens.Has( Human.Explorer ) );
