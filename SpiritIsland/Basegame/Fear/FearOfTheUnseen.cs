...
namespace SpiritIsland.Basegame {
	...
	public class FearOfTheUnseen : IFearCard {
		...
		public async Task Level2( GameState gs ) {
			foreach(var spirit in gs.Spirits) {
				var engine = new ActionEngine( spirit, gs );
				var options = spirit.Presence.Distinct().Where( s => gs.InvadersOn( s ).Has( Invader.Explorer, Invader.Town ) )
		...
		public async Task Level3( GameState gs ) {
			Space[] sacredSites = gs.Spirits.SelectMany( spirit => spirit.Presence ).Distinct().ToArray();
			foreach(var spirit in gs.Spirits) {
				var engine = new ActionEngine( spirit, gs );
