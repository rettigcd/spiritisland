...
namespace SpiritIsland.Basegame {
	...
	public class AvoidTheDahan : IFearCard {
		...
		public Task Level1(GameState gs) {
			gs.SkipExplore( gs.Island.AllSpaces.Where( s => gs.Dahan.GetCount( s ) >= 2 ).ToArray() );
		...
		public Task Level2( GameState gs ) {
			gs.SkipBuild( gs.Island.AllSpaces
				.Where( s => gs.Invaders.Counts[s].TownsAndCitiesCount() < gs.Dahan.GetCount(s)
				).ToArray()
			);
			return Task.CompletedTask;
		}
		...
		public Task Level3( GameState gs ) {
			gs.SkipBuild( gs.Island.AllSpaces
				.Where( s => 0 < gs.Dahan.GetCount( s ) ).ToArray()
			);
			return Task.CompletedTask;
		}
