...
namespace SpiritIsland.Basegame {
	...
	public class DahanRaid : IFearCard {
		...
		public Task Level2( GameState gs ) {
			return ForEachPlayerChosenLandWithDahan( gs, ( s ) => gs.SpiritFree_FearCard_DamageInvaders( s, gs.Dahan.Count(s) ) );
		}

		[FearLevel( 3, "Each player chooses a different land with Dahan. 2 Damage per Dahan there." )]
		public Task Level3( GameState gs ) {
			return ForEachPlayerChosenLandWithDahan( gs, ( s ) => gs.SpiritFree_FearCard_DamageInvaders( s, 2*gs.Dahan.Count( s ) ) );
		}
